...
function srv()
{
  ...
  #default port would be 8000 if not specified
  PORT=${1:-8000}
​
  if [ -f "${DIRNAME}/$DJANGO_SCRIPT" ]; then
    COMMAND="python ${DIRNAME}/${DJANGO_SCRIPT} runserver 0.0.0.0:${PORT}"
  elif [ -f "django/$DJANGO_SCRIPT" ]; then
    COMMAND="python django/${DJANGO_SCRIPT} runserver 0.0.0.0:${PORT}"
  elif [ -f $DJANGO_SCRIPT ]; then
    COMMAND="python ${DJANGO_SCRIPT} runserver 0.0.0.0:${PORT}"
  else
